<?php
function aawp_pcbuild_display_parts_wireless_network_adapter($atts) {
    
    $atts = shortcode_atts(array('category' => 'wireless_network_adapter'), $atts);
    $input_category = sanitize_title($atts['category']);
    
    // Define the category mapping
    $category_map = [
        'wireless_network_adapter' => 'wireless network adapters',
    ];
    
    $category = $category_map[$input_category] ?? 'wireless network adapters';
    ...
    <div style="width:90%; margin:0 auto; font-family:sans-serif;">
        <div class="pcbuilder-container" style="display:flex; gap:20px; margin-top:20px;">
            ...
            <div class="pcbuild-sidebar pcbuild-sidebar-mobile" style="width:250px; background:#f9f9f9; padding:20px; border-radius:8px;">
                ...
                <!-- protocol starting from here -->
                <div class="filter-group" style="margin-bottom: 20px; margin-top:20px;">
                    <div class="filter-header">
                        <strong>PROTOCOL</strong>
                        <button class="filter-toggle">−</button>
                    </div>
                    <div class="filter-options" id="protocol-filter">
                        <label><input type="checkbox" id="protocol-all" checked> All</label><br/>
                        <!-- Dynamic checkboxes will be injected here -->
                    </div>
                </div>
            ...
            <div class="pcbuilder-main" style="flex:1;">
                ...
                <table id="pcbuild-table">
                    ...
                    <?php include('normalized-menufracturer.php'); ?>
                    <?php include('normalize-color.php'); ?>
                    <tbody>
        ...
        <div class="product-info">
            ...
            <div class="specs-container">
                <div class="spec-group">
                    ...
                    <div class="spec-value"><?php echo esc_html($protocol); ?></div>
                </div>
                <div class="spec-group">
                ...
                <div class="spec-group">
                    ...
                    <div class="spec-value"><?php echo esc_attr($normalized_manufacturer); ?></div>
                </div>
                <div class="spec-group">
...
<!-- interface filter is starting from here -->
...
<!-- interface filter is Ending from here -->


<!-- protocol filter is starting from here -->

<script>
document.addEventListener("DOMContentLoaded", function () {
    const table = document.getElementById("pcbuild-table");
    const tableRows = table.querySelectorAll("tbody tr");
    const filterContainer = document.getElementById("protocol-filter");
    ...
    tableRows.forEach(row => {
        const protocol = row.querySelector("button.add-to-builder")?.dataset.protocol || "-";
        protocolSet.add(protocol.trim());
    });
    ...
    protocols.forEach(protocol => {
        const label = document.createElement("label");
        label.innerHTML = `<input type="checkbox" name="protocol" value="${protocol}" checked> ${protocol}`;
        label.style.display = 'block';
        checkboxElements.push(label);
    });
    ...
    const allCheckbox = document.getElementById("protocol-all");

    function updateAllCheckboxState() {
        const allBoxes = Array.from(document.querySelectorAll("input[name='protocol']"));
        const checkedBoxes = allBoxes.filter(cb => cb.checked);
        allCheckbox.checked = checkedBoxes.length === allBoxes.length;
    }

    function applyProtocolFilter() {
        const selected = Array.from(document.querySelectorAll("input[name='protocol']:checked"))
            .map(cb => cb.value);

        tableRows.forEach(row => {
            const protocol = row.querySelector("button.add-to-builder")?.dataset.protocol?.trim();
            row.style.display = selected.includes(protocol) ? "" : "none";
        });
    ...
    allCheckbox.addEventListener("change", function () {
        const allBoxes = document.querySelectorAll("input[name='protocol']");
        allBoxes.forEach(cb => cb.checked = allCheckbox.checked);
        applyProtocolFilter();
    });
...
<!-- protocol filtering is ending -->


<!-- color filtering is starting from here -->
<script>
document.addEventListener("DOMContentLoaded", function () {
    const table = document.getElementById("pcbuild-table");
    const tableRows = table.querySelectorAll("tbody tr");
    ...
    tableRows.forEach(row => {
        const color = row.querySelector("button.add-to-builder")?.dataset.color || "-";
    ...
    function applyColorFilter() {
        ...
        tableRows.forEach(row => {
            const color = row.querySelector("button.add-to-builder")?.dataset.color?.trim();
...
add_shortcode('pcbuild_parts_wireless_network_adapter', 'aawp_pcbuild_display_parts_wireless_network_adapter');
