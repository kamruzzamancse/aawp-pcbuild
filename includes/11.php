...
function aawp_pcbuild_display_parts_gpu($atts) {
            ...
            <div style="flex:1;">
                ...
                <table id="pcbuild-table" style="width:100%; border-collapse:collapse;">
                    ...
                    <?php foreach ($display_items as $index => $item):
                        ...
                        <tr class="gpu-row" style="background-color: <?php echo $row_bg; ?>; border-bottom:1px solid #DDD; font-size: 14px"
                            ...
                            data-rating="<?php echo esc_attr($rating_display); ?>"
    ...
    <script type="text/javascript">
        // Compatibility filtering based on selected CPU
        function filterByCompatibility() {
            var selected_cpu = localStorage.getItem('selected_cpu');  // Get selected CPU from localStorage
            var rows = document.querySelectorAll('.gpu-row');
            
            if (selected_cpu) {
                rows.forEach(row => {
                    var compatible = row.getAttribute('data-compatibility').includes(selected_cpu);
                    row.style.display = compatible ? '' : 'none';  // Hide non-compatible GPUs
                });
            } else {
                rows.forEach(row => row.style.display = '');  // Show all if no CPU is selected
            }
        }

        // Run the compatibility filter when page is loaded
        document.addEventListener('DOMContentLoaded', filterByCompatibility);
    </script>
